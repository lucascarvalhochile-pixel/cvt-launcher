...
# ═══════════════════════════════════════════════════════
# LCX PRICES SYNC (Fase 2 — 08/07/2026)
# Endpoint pra WP consumir seasonPrices dos tours LCX.
# ═══════════════════════════════════════════════════════

TOURS_CACHE = {"data": {}, "ts": None, "count": 0}
TOURS_API_KEY = os.environ.get("TOURS_API_KEY", "lc-turismo-tours-2026-changeMe")
TOURS_REFRESH_HOURS = int(os.environ.get("TOURS_REFRESH_HOURS", "24"))


def _lcx_get_tours_page(search, page):
    """Wrapper defensivo pra getTours action."""
    try:
        if not lcx_client.logged_in:
            if not lcx_client.login():
                return None
        payload = json.dumps([{"search": search, "page": page}])
        r = lcx_client.session.post(
            f"{LCX_BASE}/dashboard/vendas/nova",
            data=payload,
            headers={
        ...
        for line in r.text.split("\n"):
            if '"success":true' in line and '"data":' in line:
                idx = line.find('{"success":true')
                j = line[idx:]
                depth = 0
                for i, c in enumerate(j):
                    if c == "{": depth += 1
                        ...
                        depth -= 1
                        if depth == 0:
                            try: return json.loads(j[:i+1])
                            except: return None
    except Exception as e:
        print(f"[PRICES] getTours err {search} p{page}: {e}")
    return None


def refresh_tours_cache():
    """Coleta TODOS os tours ativos + seasonPrices do LCX."""
    print("[PRICES] Refreshing tours cache from LCX...")
    all_tours = {}
    # Varredura por prefix + letras comuns (garante coverage)
    searches = ["", "a", "e", "i", "o", "s", "r", "t", "n",
                "CHISAN", "CHIATA", "CHIUYU", "COLCAR", "COLSAO",
                "PERLIM", "PERCUS", "ARG", "MEX", "USA", "DOM"]
    for search in searches:
        for page in range(1, 30):
            data = _lcx_get_tours_page(search, page)
            if not data or not data.get("data"):
                break
            for t in data["data"]:
                # Só ativos com seasonPrices
                if t.get("status") == "active":
                    all_tours[t["id"]] = {
                        ...
                        "seasonPrices": t.get("seasonPrices") or [],
                    }
            pag = data.get("pagination", {})
            if page >= pag.get("totalPages", 1):
                break
    TOURS_CACHE["data"] = all_tours
    TOURS_CACHE["ts"] = datetime.now().isoformat()
    TOURS_CACHE["count"] = len(all_tours)
    print(f"[PRICES] Cache refreshed: {len(all_tours)} tours")
    return len(all_tours)


def tours_refresh_worker():
    """Background thread: refresh a cada TOURS_REFRESH_HOURS horas."""
    # Inicial delay pra não competir com scan
    time.sleep(60)
    while True:
        try:
            refresh_tours_cache()
        except Exception as e:
            print(f"[PRICES] Refresh err: {e}")
        time.sleep(TOURS_REFRESH_HOURS * 3600)


@app.route("/api/lcx_prices")
def api_lcx_prices():
    """Retorna dict completo de tours + seasonPrices. Autenticado por ?key=X."""
    if request.args.get("key") != TOURS_API_KEY:
        return jsonify({"error": "unauthorized"}), 401
    return jsonify({
        "count": TOURS_CACHE["count"],
        "refreshed_at": TOURS_CACHE["ts"],
        "tours": list(TOURS_CACHE["data"].values()),
    })


@app.route("/api/lcx_prices/<cuid>")
def api_lcx_prices_one(cuid):
    """Retorna seasonPrices de um tour por CUID."""
    if request.args.get("key") != TOURS_API_KEY:
        return jsonify({"error": "unauthorized"}), 401
    tour = TOURS_CACHE["data"].get(cuid)
    if not tour:
        return jsonify({"error": "not found", "refreshed_at": TOURS_CACHE["ts"]}), 404
    return jsonify(tour)


@app.route("/api/lcx_prices_refresh", methods=["POST"])
def api_lcx_prices_refresh():
    """Força refresh manual do cache."""
    if request.args.get("key") != TOURS_API_KEY:
        return jsonify({"error": "unauthorized"}), 401
    n = refresh_tours_cache()
    return jsonify({"refreshed": n, "ts": TOURS_CACHE["ts"]})


# Start tours refresh thread
_prices_thread = threading.Thread(target=tours_refresh_worker, daemon=True)
_prices_thread.start()
print(f"[PRICES] Refresh thread started (interval: {TOURS_REFRESH_HOURS}h)")
