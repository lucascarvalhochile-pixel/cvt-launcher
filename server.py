...
# Action IDs - updated 2026-06-09 after LCX redeploy (dpl_YwEbVwwjvcVVW892wdc5uS7KHfi6)
ACTION_CREATE_SALE = "6064f03c992e93700e26a7a0a2d122face1e1248b4"
ACTION_GET_TOURS = "402570c07fb47a41f59c113545c4828f99602443eb"
...
def find_lcx_tour(atividade, codigo_interno, cidade=""):
    """Find LCX tour code from Civitatis activity name or internal code.

    When cidade is provided, cross-validates that the matched tour's country
    prefix (CHI=Chile, COL=Colômbia, PER=Peru) matches the email's city country.
    This prevents e.g. 'Trilha pela Montanha Arco-Íris' from Cusco (Peru)
    matching CHIATA020 (Atacama, Chile) instead of PERCUS010 (Cusco, Peru).
    """
    mapping = load_mapping()
    if not mapping:
        return None, None

    # Resolve country from email city for cross-validation
    email_country = ""
    if cidade:
        email_country, _ = resolve_country_city(cidade)

    # Map LCX code prefix to country name
    PREFIX_COUNTRY = {"CHI": "Chile", "COL": "Colômbia", "PER": "Peru"}

    def _country_ok(codigo_lcx):
        """Check if tour code country matches email city country.
        If no city was provided or country can't be determined, allow the match."""
        if not email_country or not codigo_lcx or len(codigo_lcx) < 3:
            return True
        tour_country = PREFIX_COUNTRY.get(codigo_lcx[:3].upper(), "")
        return not tour_country or tour_country == email_country

    key = unicodedata.normalize("NFC", atividade.strip().lower())
    # Remove language/tier suffix: " - Tour em português", " - Tour com retirada + ..."
    key_clean = re.sub(r'\s*-\s*tour\s+.*
...
def build_lcx_sale(parsed_email):
    ...
    # Find LCX tour mapping (cidade enables country cross-validation)
...
, '', key, flags=re.IGNORECASE).strip()
    ...
    if key_clean in mapping:
        m = mapping[key_clean]
        if _country_ok(m["codigo_lcx"]):
            return m["codigo_lcx"], m["nome_lcx"]
        # Country mismatch — log and try other strategies
        app.logger.warning(f"find_lcx_tour: exact match '{key_clean}' → {m['codigo_lcx']} SKIPPED (country mismatch: tour={m['codigo_lcx'][:3]}, email_city={cidade}→{email_country})")
    ...
    for k, v in mapping.items():
        if _strip_accents(k) == key_no_accent:
            if _country_ok(v["codigo_lcx"]):
                return v["codigo_lcx"], v["nome_lcx"]
            app.logger.warning(f"find_lcx_tour: accent match '{k}' → {v['codigo_lcx']} SKIPPED (country mismatch)")

    # 2. Match via código interno (accent-insensitive) — ALWAYS try this, no country filter
    #    código interno is the most reliable identifier from Civitatis
    if codigo_interno:
